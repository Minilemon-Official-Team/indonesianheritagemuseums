...
export default function AutoGuide() {
    ...
    useEffect(() => {
        museumData.forEach((zone) => {
            const obj = zone.objects[0];
            if (obj) prefetchTranslation(zone.name, obj.description);
        });
    ...
    return (
        <div className="bg-[#F4EFE6] min-h-screen">
            ...
            <div className="max-w-[1200px] mx-auto px-4 py-12">
                {museumData.map((zone, index) => {
                    const obj = zone.objects[0];
                    const description = getDescription(zone.name, obj.description);

                    return (
                        <section key={index} className="mb-16">
                            <h2 className="font-['Cinzel'] text-3xl text-[#8C6B3E] mb-6">{zone.name}</h2>

                            <div
                                id={`object-${obj.id}`}
                                ref={(el) => {
                                    objectRefs.current[`object-${obj.id}`] = el;
                                }}
                            ...
                            >
                                <button
                                    ...
                                    aria-label="Share"
                                >
                                    {copiedId === obj.id ? <Check className="w-4 h-4" /> : <Share className="w-4 h-4" />}
                                </button>
